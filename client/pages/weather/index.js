...
Page({
  ...
  dataCache() {
    const { current, backgroundColor, backgroundImage, today, tomorrow, address, hourlyData } = this.data
    wx.setStorage({
      key: 'defaultData',
      data: {
        current,
    ...
    wx.getStorage({
      ...
      success: ({ data }) => {
        if (data && !isUpdate) {
          ...
          const { current, backgroundColor, backgroundImage, today, tomorrow, address, hourlyData } = data
          this.setData({
            current,
            backgroundColor,
            backgroundImage,
